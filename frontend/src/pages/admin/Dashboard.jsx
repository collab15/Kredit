...
const TX_COLS = [
  { key: 'type',       label: 'Type',   render: (r) => (
    <span className={`k-badge ${r.type === 'reward' ? 'bg-kred/10 text-kred' : r.type === 'org_payment' ? 'bg-cyan-500/10 text-cyan-400' : 'bg-accent/10 text-accent'}`}>{r.type}</span>
  )},
...
export default function AdminDashboard() {
  ...
  useEffect(() => {
    Promise.all([transactionsApi.getStats(), transactionsApi.getAll(), favoursApi.getAll()])
  ...
  return (
    <div className="p-8 space-y-8 animate-fade-in">
      <div>
        <h1 className="font-mono text-2xl font-bold text-white tracking-tight">Admin Dashboard</h1>
        <p className="text-muted text-sm mt-1">Full network overview</p>
      </div>
