...
export default function AdminFavours() {
  ...
  const [requestorUser, setRequestorUser] = useState(null);
  ...
  const load = () => {
    setLoading(true);
    favoursApi.getAll().then(r => setFavours(r.data)).finally(() => setLoading(false));
  };
  useEffect(load, []);
  ...
  const handleCreate = async (e) => {
    e.preventDefault();
    if (!requestorUser) { toast.error('Please enter a valid requestor username'); return; }
    if (!requesteeUser) { toast.error('Please enter a valid requestee username'); return; }
  ...
  return (
    <div className="p-8 space-y-6 animate-fade-in">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="font-mono text-2xl font-bold">Favours</h1>
          ...
          <form onSubmit={handleCreate} className="space-y-4">
            <div>
              ...
              <UserLookupInput
                ...
                onResolved={setRequestorUser}
                excludeId={requesteeUser?.user_id}
                placeholder="Who is requesting…"
                required
              />
            </div>
            <div>
              <label className="k-label">Requestee Username <span className="text-danger">*</span></label>
              <UserLookupInput
                value={requesteeName}
                onChange={setRequesteeName}
                onResolved={setRequesteeUser}
                excludeId={requestorUser?.user_id}
                placeholder="Who will do the favour…"
                required
              />
            </div>
            <div>
            ...
            <div className="flex justify-end gap-2 pt-2">
              ...
              <button type="submit" className="k-btn-primary" disabled={submitting || !requestorUser || !requesteeUser}>
      ...
      {showComplete && (
        <Modal title="Complete Favour" onClose={() => { setShowComplete(false); setCompletion(EMPTY_COMPLETE); }}>
          <form onSubmit={handleComplete} className="space-y-4">
            <div className="p-3 rounded-lg bg-kred/5 border border-kred/20 text-sm text-muted">Mark this favour as done. Optionally leave a review.</div>
            <div>
              <label className="k-label">Review (optional)</label>
              <textarea className="k-input resize-none h-20" placeholder="Great help…"
