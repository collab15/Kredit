...
export default function AdminFavours() {
  ...
  const counts   = { all: favours.length, pending: favours.filter(f => f.status === 'pending').length, completed: favours.filter(f => f.status === 'completed').length, open: favours.filter(f => f.status === 'open').length };

  const handleCreate = async (e) => {
    e.preventDefault(); setSubmitting(true);
    try { await favoursApi.create(form); toast.success('Favour created!'); setShowCreate(false); setForm(EMPTY_FAVOUR); load(); }
    catch (err) { toast.error(err.message); } finally { setSubmitting(false); }
  };

  const handleComplete = async (e) => {
    e.preventDefault(); setSubmitting(true);
    try { await favoursApi.complete(completion.favour_id, { review: completion.review }); toast.success('Favour completed!'); setShowComplete(false); setCompletion(EMPTY_COMPLETE); load(); }
    catch (err) { toast.error(err.message); } finally { setSubmitting(false); }
  };
  ...
  const columns = [
    { key: 'requestor',   label: 'Requestor',   render: (r) => <span className="font-mono text-white">{r.requestor}</span> },
    { key: 'requestee',   label: 'Requestee',   render: (r) => <span className="font-mono text-slate-300">{r.requestee}</span> },
    { key: 'description', label: 'Description', render: (r) => r.description ? <span className="text-slate-400 text-xs">{r.description}</span> : <span className="text-muted">—</span> },
    ...
    { key: 'review',      label: 'Review',      render: (r) => r.review ? <span className="text-xs text-slate-400 italic">"{r.review}"</span> : <span className="text-muted">—</span> },
  ...
  return (
    <div className="p-8 space-y-6 animate-fade-in">
      <div className="flex items-center justify-between">
        ...
        <button onClick={() => setShowCreate(true)} className="k-btn-primary flex items-center gap-2"><Plus size={14} /> New Favour</button>
      </div>
      ...
      {showCreate && (
        <Modal title="Create Favour" onClose={() => { setShowCreate(false); setForm(EMPTY_FAVOUR); }}>
          <form onSubmit={handleCreate} className="space-y-4">
            <div>
              <label className="k-label">Requestor <span className="text-danger">*</span></label>
              <select className="k-input" required value={form.requestor_id} onChange={e => setForm(p => ({ ...p, requestor_id: e.target.value }))}>
                <option value="">Select user…</option>
                {users.map(u => <option key={u.user_id} value={u.user_id}>{u.username}</option>)}
              </select>
            </div>
            <div>
              <label className="k-label">Requestee <span className="text-danger">*</span></label>
              <select className="k-input" required value={form.requestee_id} onChange={e => setForm(p => ({ ...p, requestee_id: e.target.value }))}>
                <option value="">Select user…</option>
                {users.filter(u => u.user_id !== form.requestor_id).map(u => <option key={u.user_id} value={u.user_id}>{u.username}</option>)}
              </select>
            </div>
            <div>
              <label className="k-label">Description</label>
              <textarea className="k-input resize-none h-20" placeholder="Help me move furniture…" value={form.description} onChange={e => setForm(p => ({ ...p, description: e.target.value }))} />
            </div>
            <div className="flex justify-end gap-2 pt-2">
              <button type="button" className="k-btn-ghost" onClick={() => setShowCreate(false)}>Cancel</button>
              <button type="submit" className="k-btn-primary" disabled={submitting}>{submitting ? 'Creating…' : 'Create'}</button>
            </div>
          </form>
        </Modal>
      )}
      ...
      {showComplete && (
        <Modal title="Complete Favour" onClose={() => { setShowComplete(false); setCompletion(EMPTY_COMPLETE); }}>
          <form onSubmit={handleComplete} className="space-y-4">
            <div className="p-3 rounded-lg bg-kred/5 border border-kred/20 text-sm text-slate-300">Mark this favour as done. Optionally leave a review.</div>
            <div>
              <label className="k-label">Review (optional)</label>
              <textarea className="k-input resize-none h-20" placeholder="Great help…" value={completion.review} onChange={e => setCompletion(p => ({ ...p, review: e.target.value }))} />
            </div>
            <div className="flex justify-end gap-2 pt-2">
