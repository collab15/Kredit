  ...
  return (
    <div className="p-8 space-y-6 animate-fade-in">
      ...
      <div className="k-card p-5 flex items-center gap-4 max-w-md">
        ...
        <div>
          <p className="font-semibold text-white text-lg">{profile?.name || profile?.delegate || 'Unnamed Org'}</p>
          <span className={`k-badge mt-1 inline-block ${isAgency ? 'bg-violet-500/10 text-violet-400' : 'bg-cyan-500/10 text-cyan-400'}`}>
            {user?.org_type}
          </span>
          <p className="text-[11px] text-muted mt-1">
            {isAgency ? 'Issues unlimited kreds to users' : 'Accepts kreds from users'}
          </p>
        </div>
      </div>

      {/* API Key (login credential) */}
      <div className="k-card p-4 max-w-md">
        <p className="text-[10px] text-muted uppercase tracking-widest mb-2">API Key — use this to log in</p>
      ...
      <div className="max-w-lg k-card p-6">
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          {isAgency
            ? <Field label="Scope (what your agency covers)" name="scope" state={form} setState={setForm} placeholder="Community Service, Education" />
            : <Field label="Services offered"               name="services" state={form} setState={setForm} placeholder="Healthcare, Transport, Food" />
          }
